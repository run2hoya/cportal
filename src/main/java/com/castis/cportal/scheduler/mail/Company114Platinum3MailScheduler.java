...
public class Company114Platinum3MailScheduler {
	...
	@Scheduled(cron="${company114Platinum3MailScheduler.cron:0 0 8 * * 1}")
	protected void executeInternal() throws JobExecutionException {
		...
		try {
			...
			if(properties.getPlatinum3() != 0) {
				Company company = companyService.getCompany(properties.getPlatinum3());
			...
			log.error("",e);			
		} finally {log.info("Company114Platinum1MailScheduler - end(Processing time: " + (System.currentTimeMillis() - start) /1000.0 + " s)");}
